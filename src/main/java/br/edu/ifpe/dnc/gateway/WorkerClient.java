...
public class WorkerClient {
    ...
    public String sendAnalysis(WorkerInfo worker, String networkJson) {
        ...
        try {
            HttpRequest request = HttpRequest.newBuilder()
                    ...
                    .POST(HttpRequest.BodyPublishers.ofString(networkJson))
                    .build();
            ...
            if (response.statusCode() >= 200 && response.statusCode() < 300) {
                ...
                worker.markOffline();
                return "{\"error\": \"Worker " + worker.getId() + " returned HTTP " + response.statusCode() + "\"}";
            }

        } catch (Exception e) {
            worker.markOffline();
            return "{\"error\": \"Worker " + worker.getId() + ": " + e.getMessage() + "\"}";
        } finally {
